...
export default {
  async fetch(request) {
    ...
    /* 全部数据 */
    if (path === '/' || path === '/api/data' || path === '/api') {
      return json(DATA);
    }
    ...
    /* 今日日期 */
    if (path === '/api/today') {
      return json({ today: DATA.today });
    }

    /* 按品类过滤时间轴 */
    const catMatch = path.match(/^\/api\/timeline\/(\w+)$/);
    if (catMatch) {
      const cat = catMatch[1];
      var typeToKey = DATA.typeToKey;
      var filtered = DATA.timeline.map(function (day) {
        var items = day.items.filter(function (it) {
          return typeToKey[it.type] === cat;
        });
        return items.length > 0 ? Object.assign({}, day, { items: items }) : null;
      }).filter(Boolean);
    ...
    return json({ error: 'Not found', endpoints: [
      'GET /api/data', 'GET /api/categories', 'GET /api/carousel',
      'GET /api/timeline', 'GET /api/timeline/:category', 'GET /api/today'
    ] }, 404);
  },
};
